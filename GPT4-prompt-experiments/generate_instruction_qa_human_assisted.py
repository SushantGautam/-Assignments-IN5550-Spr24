...
def parse_args():
    ...
    parser.add_argument("--Creative", required=False, type=bool, help="Generate Creative Based QA pairs.", default=True)
    parser.add_argument("--Summary", required=False, type=bool, help="Generate Summary Based QA pairs.", default=True)
    parser.add_argument("--Caption", required=False, type=bool, help="Generate Caption Based QA pairs.", default=True)
...
def annotate(gt_file, caption_files, output_dir, args):
    ...
    for file in tqdm(caption_files):
        ...
        with open(json_file_path, "w") as f:
            print(f"Saving annotations for {key} in {json_file_path}")
...
def main():
    ...
    caption_files_creative = [f'{video_id}_creative.json' for video_id in caption.keys()]
    caption_files_caption = [f'{video_id}_caption.json' for video_id in caption.keys()]
    caption_files_summary = [f'{video_id}_summary.json' for video_id in caption.keys()]
    caption_files_summary = [f'{video_id}_summary.json' for video_id in caption.keys()]
    caption_files = caption_files_creative + caption_files_caption + caption_files_summary
